...
export class DescriptionBorderComponent implements OnInit {
  ...
  saveEdit() {
    // 向数据库发送数据
    this.homeServicce.changeInformation(this.datas).subscribe((data) => {
      if (data.code == 200) {
  ...
  handleChange(file: UploadFile): void {
    ...
    this.loading = true;
    this.homeServicce
      .uploadImg(file, this.datas.u_id)
      .subscribe((data: any) => {
  ...
  getUser(u_id: string) {
    this.homeServicce.getUser(u_id).subscribe((data) => {
      if (data.code == 200) {
  ...
  resetAvatar() {
    // 将头像恢复为默认的头像
    this.homeServicce
      .uploadImg(null, this.datas.u_id)
      .subscribe((data: any) => {
        if (data.body.code == 200) {
